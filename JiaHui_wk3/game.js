...
const rooms=[
 ...
 {name:'The Library',description:'An expansive chamber filled with ancient tomes and whispered secrets.',color:'#4a5a6e',code:'3847',puzzles:[
 ["The ancient tome", "A classic riddle: I speak without a mouth and hear without ears. I have no body, but I come alive with wind. What am I?", "echo", "Think of natural phenomena that respond to sound."],
 ["The cipher scroll", "Using a Caesar cipher with shift 3, decode: WKH DQVZHU LV ILYH.", "the answer is five", "Shift each letter back 3 positions in the alphabet."],
 ["The book spines", "Six books are arranged by color: Red, Blue, Green, Yellow, Orange, Purple. Red is left of Blue. Green is not at either end. Yellow is between Orange and Blue. Where is Purple?", "right", "Use process of elimination with each constraint."]]},
 {name:'The Servants Room',description:'A modest chamber where the household staff would rest between duties.',color:'#6b5d4f',code:'7521',puzzles:[
 ["The servant's journal", "Two numbers sum to 15. Their product is 54. What is the larger number?", "nine", "Set up equations: x + y = 15 and x × y = 54."],
 ["The old key", "I am an odd number. Take away one letter and I become even. What number am I?", "seven", "The word 'seven' has the letter 's' which sounds like 'even'."],
 ["The duty roster", "Three servants work shifts. Alice works every 2 days, Bob every 3, Carol every 5. If all worked today, in how many days will all three work together again?", "thirty", "Find the least common multiple of 2, 3, and 5."]]},
 {name:'The Grand Living Room',description:'An ornate hall of mirrors and grand tapestries. The final chamber awaits.',color:'#8b7355',code:'9264',puzzles:[
 ["The grand piano", "I have keys but no locks. I have space but no room. What am I?", "keyboard", "Think of objects with musical or technological significance."],
 ["The tapestry pattern", "If 3 painters can paint 3 rooms in 3 hours, how many hours will 9 painters take to paint 9 rooms?", "three", "The rate per painter remains constant."],
 ["The mirror puzzle", "A number is 5 times another number. Their sum is 60. Find the smaller number.", "ten", "Set up: x + 5x = 60, so 6x = 60, thus x = 10."]]}
];
...
const codeClues=[
 ...
 [
  {place:'Inside a grimoire',p:[-4.5, 2.8, 5.2],text:"FIRST DIGIT · How many days are in 13 weeks?",hint:"Multiply 13 by 7."},
  {place:'On a map',p:[2.3, 2.1, 3.5],text:"SECOND DIGIT · The units digit of 7 squared.",hint:"Calculate 7 × 7 and look at the last digit."},
  {place:'In a locked box',p:[-1.2, 1.9, 8.1],text:"THIRD DIGIT · How many sides does an octagon have?",hint:"The prefix 'octa' refers to eight."},
  {place:'Under the reading desk',p:[5.1, 1.6, 2.3],text:"FOURTH DIGIT · What is 12 ÷ 3?",hint:"Basic division."}
 ],
 [
  {place:'In a pillowcase',p:[-5.5, 1.8, 6.2],text:"FIRST DIGIT · How many hours in a full day?",hint:"Count the hours from midnight to midnight."},
  {place:'Behind the portrait',p:[3.8, 3.1, 1.9],text:"SECOND DIGIT · How many sides on two triangles combined?",hint:"Each triangle has 3 sides."},
  {place:'Under a cushion',p:[-2.1, 1.5, 7.8],text:"THIRD DIGIT · What is 6 - 5?",hint:"Simple subtraction."},
  {place:'Inside a drawer',p:[6.2, 2.2, 4.5],text:"FOURTH DIGIT · How many prime numbers are between 0 and 10?",hint:"They are: 2, 3, 5, 7."}
 ],
 [
  {place:'On the window ledge',p:[-6.8, 2.5, 3.1],text:"FIRST DIGIT · What is 18 ÷ 2?",hint:"Basic division."},
  {place:'Inside an urn',p:[1.5, 2.9, 6.8],text:"SECOND DIGIT · How many corners does a cube have?",hint:"A cube is a 3D square with 8 vertices."},
  {place:'Behind a mirror',p:[-0.9, 2.1, 2.4],text:"THIRD DIGIT · What is 3 + 3 + 3?",hint:"Add three times."},
  {place:'Under the grand chandelier',p:[4.9, 3.5, 5.2],text:"FOURTH DIGIT · How many letters in the word FOUR?",hint:"Count the letters: F-O-U-R."}
 ]
];
const $=s=>document.querySelector(s),canvas=$('#scene'),ctx=createRoomRenderer(canvas);
let playing=false,hasSave=false;
let discoveries={},room=0,found=[],journal=[],active=null,finished=false,timeLimit=180,timeElapsed=0,gameOver=false; const storage='house-of-three-v1';
try{const s=JSON.parse(localStorage.getItem(storage));if(s&&Number.isInteger(s.room)&&s.room>=0&&s.room<7&&Array.isArray(s.found)&&Array.isArray(s.journal)){hasSave=true;room=s.room;found=[...new Set(s.found.filter(i=>Number.isInteger(i)&&i>=0&&i<3))];journal=s.journal.filter(n=>n&&typeof n.room==='string'&&typeof n.title==='string'&&typeof n.answer==='string');for(let r=0;r<7;r++){const entries=s.discoveries?.[r];if(Array.isArray(entries))discoveries[r]=[...new Set(entries.filter(i=>Number.isInteger(i)&&i>=0&&i<4))];}finished=!!s.finished;}}catch{}
...
const keyPositions=[
 ...
 [[-6,1.8,2.5],[0,1.5,6.8],[5.5,2.1,4.1]],
 [[-5.2,1.6,7.2],[3.1,2.3,2.8],[-2.8,1.9,5.5]],
 [[-4.5,2.2,3.9],[4.8,1.8,6.5],[1.2,2.6,1.8]]
];
...
function setup(){camera={x:0,y:2.5,z:13.8,a:0,pitch:-.08};objects=[];const r=rooms[room];box(0,-.16,8.5,20,.3,19,'#776c59');box(0,4,-.45,20,8,.3,r.color);box(-10,4,8.5,.2,8,19,r.color);box(10,4,8.5,.2,8,19,r.color);
 ...
 if(room===4){box(-4.5,1.2,3,3.2,1.8,2.5,'#5a4a3a');for(let i=0;i<8;i++)box(-5+i*1.4,.9,2,.08,3,.5,i%2?'#6b5a4a':'#7a6a5a');box(1.5,1.5,1,1.8,2.2,1.2,'#544538');}
 if(room===5){box(-3.2,.6,2.5,2.5,1.4,1.8,'#7a6b5d');box(2.8,1.1,2,2,1.6,1.5,'#6a5a4a');box(-1,1.8,3.5,1.2,.6,.8,'#8a7a6a');}
 if(room===6){box(-4,1.3,2,3.5,2,2.2,'#8a7a6a');box(2.5,1.5,1.5,1.8,1.8,1.5,'#9a8a7a');for(let i=0;i<4;i++)box(-2+i*.8,2.5,3.2,.15,1.5,.1,'#c4b4a4');}
 ...
 $('#roomName').textContent=r.name;$('#description').textContent=r.description;$('#number').textContent=`0${room+1} — 07`;$('#keys').innerHTML=[0,1,2].map(i=>`<span class="key ${found.includes(i)?'found':''}" aria-label="Key ${i+1} ${found.includes(i)?'found':'missing'}">${found.includes(i)?'⚿':'·'}</span>`).join('');
 const mins=Math.floor(timeLimit/60);const secs=Math.ceil(timeLimit%60);if($('#timer'))$('#timer').textContent=`${mins}:${String(secs).padStart(2,'0')}`;
 updateClueStatus();
...
function render(t){
 ...
 if(!document.hidden){
  if(playing&&!gameOver&&!$('#modal').open)movePlayer(dt);
  if(playing&&!gameOver){timeElapsed+=dt;timeLimit-=dt;if(timeLimit<=0){timeLimit=0;gameOver=true;$('#modal').close();showTimeOut();}}
...
function showTimeOut(){active=null;$('#kind').textContent='TIME EXPIRED';$('#title').textContent='The manor has locked its doors.';$('#body').textContent='You ran out of time. The mysteries remain unsolved, and Blackthorn Manor keeps its secrets.';const b=document.createElement('button');b.textContent='Try again';b.onclick=()=>{$('#modal').close();room=0;found=[];journal=[];discoveries={};finished=false;gameOver=false;timeLimit=180;timeElapsed=0;save();setup();};$('#body').append(document.createElement('br'),b);$('#answerForm').hidden=true;$('#hint').hidden=true;$('#feedback').textContent='';$('#modal').showModal();}
...
$('#answerForm').onsubmit=e=>{e.preventDefault();if(!playing||gameOver||!Number.isInteger(active)||![0,1,2,4].includes(active))return;const a=$('#answer').value.trim().toLowerCase().replace(/[.!?]$/,'');if(active===4){if(found.length!==3){$('#feedback').textContent='The three key locks must be opened first.';return;}if(a!==rooms[room].code){$('#feedback').textContent='That code does not fit. Compare the fragments in your notebook.';return;}$('#modal').close();if(room===6){finished=true;save();win();}else{room++;found=[];save();setup();notify('Door unlocked. A new room, three new keys.');}}else if(active<3){if(a===rooms[room].puzzles[active][2]){if(!found.includes(active)){found.push(active);journal.push({room:rooms[room].name,title:rooms[room].puzzles[active][0],answer:a});save();}$('#modal').close();const previousCamera={...camera};setup();camera=previousCamera;notify(`Key secured · ${found.length} of 3`);}else $('#feedback').textContent='The lock stays shut. Check your reasoning and try again.';}}
...
function win(){active=null;$('#kind').textContent='07 / 07 · BLACKTHORN MANOR RELEASES YOU';$('#title').textContent='You found your way out in time.';$('#body').textContent='Twenty-one keys. Seven codes. Every room understood.\n\nThe great doors open onto the manor's sweeping grounds. Beyond the fountain and iron gates, morning waits. Blackthorn Manor finally lets you go—and the clock stops ticking.';const b=document.createElement('button');b.textContent='Play again';b.onclick=()=>{room=0;found=[];journal=[];discoveries={};finished=false;gameOver=false;timeLimit=180;timeElapsed=0;save();$('#modal').close();setup();};$('#body').append(document.createElement('br'),b);$('#answerForm').hidden=true;$('#hint').hidden=true;$('#feedback').textContent='';$('#modal').showModal();}
let drag=null;
...
function showHome(){playing=false;resetInput();$('#gameUI').hidden=true;$('#homepage').hidden=false;document.body.classList.add('at-home');$('#continueBtn').hidden=!hasSave;$('#continueBtn').textContent=finished?'View your completed escape →':'Continue your escape →';$('#playBtn').innerHTML=hasSave?'Start a new escape <span>↗</span>':'Enter the mansion <span>↗</span>';$('#saveInfo').textContent=hasSave?(finished?'All seven rooms completed. You beat the clock!':`${rooms[room].name} · ${found.length} of 3 keys · ${foundClues().length} of 4 code clues · ${Math.floor(timeLimit)}s remaining`):'3-minute race against time. Enter if you dare.';$('.home-caption').textContent='BLACKTHORN MANOR / THE ESTATE AWAITS';$('#playBtn').focus();}
function enterHouse(fresh){if(fresh){room=0;found=[];journal=[];discoveries={};finished=false;gameOver=false;timeLimit=180;timeElapsed=0;save();}playing=true;resetInput();$('#homepage').hidden=true;$('#gameUI').hidden=false;document.body.classList.remove('at-home');setup();$('#notes').focus();if(finished)win();}
...
$('#howBtn').onclick=()=>{active=null;$('#kind').textContent='A FEW THINGS BEFORE YOU ENTER';$('#title').textContent='Look closer.';$('#body').textContent='Explore seven grand chambers in Blackthorn Manor: the bathroom, living room, bedroom, kitchen, library, servants room, and grand living room. Click glowing markers to examine objects and solve a mix of word riddles, logic challenges, and math puzzles hiding three keys in each room.\n\nFind four passcode fragments in different places around each room. Each fragment gives one digit and its position. Discovered clues are copied into your notebook; undiscovered clues stay hidden. You need all three keys AND the correct code to open the exit to the next room.\n\nYou have 3 minutes to escape the entire manor! WASD to move · Arrow keys or drag to look · J for notebook · On a phone, use the on-screen arrows.\n\nHints are available in each puzzle. Your progress saves automatically. Good luck—the clock is ticking!';$('#answerForm').hidden=true;$('#hint').hidden=true;$('#feedback').textContent='';$('#modal').showModal();};
