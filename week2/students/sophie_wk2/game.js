...
scene.add(new THREE.HemisphereLight(0x789bd1,0x281b35,1.35));scene.add(new THREE.AmbientLight(0x826f9d,.38));const moonLight=new THREE.DirectionalLight(0xaec9ff,3.3);moonLight.position.set(-20,38,18);moonLight.castShadow=true;moonLight.shadow.mapSize.set(2048,2048);moonLight.shadow.camera.left=-25;moonLight.shadow.camera.right=25;moonLight.shadow.camera.top=65;moonLight.shadow.camera.bottom=-10;moonLight.shadow.bias=-.0005;scene.add(moonLight);

const mats={stone:new THREE.MeshStandardMaterial({color:0x34465b,roughness:.75}),edge:new THREE.MeshStandardMaterial({color:0x738b91,roughness:.7}),pink:new THREE.MeshStandardMaterial({color:0xf08fae,roughness:.58}),palePink:new THREE.MeshStandardMaterial({color:0xffd1df,roughness:.62}),deepPink:new THREE.MeshStandardMaterial({color:0xcf5f88,roughness:.58}),dark:new THREE.MeshStandardMaterial({color:0x442c3d}),gold:new THREE.MeshStandardMaterial({color:0xffc32f,emissive:0xffa000,emissiveIntensity:5.5,roughness:.35})};
...
[-8,12,7,5,1,4],[-10,14,0,4,1,4],[-5,16,-7,4.5,1,4],[3,18,-9,5,1,3.8],[10,20,-5,3.8,1,4.5],
[11,22,3,3.6,1,3.6],[6,24,10,4.2,1,3.5],[-2,26,11,4.5,1,3.4],[-9,28,7,3.5,1,3.5],[-11,30,-1,3.4,1,5],
[-7,32,-9,3.7,1,3.5],[1,34,-11,4.2,1,3.4],[9,36,-7,3.3,1,3.3],[11,38,1,3.2,1,4.6],[7,40,9,3.7,1,3.3],
[0,42,11,4,1,3.2],[-8,44,8,3.2,1,3.2],[-11,46,0,3.1,1,4],[-6,48,-8,3.4,1,3.1],[2,50,-10,4,1,3.5],[0,53,0,13,1,13]
...
lampSpots.forEach(([x,y,z],index)=>{const lamp=new THREE.Group(),pole=new THREE.Mesh(new THREE.CylinderGeometry(.06,.08,1.7,8),mats.dark),bulb=new THREE.Mesh(new THREE.SphereGeometry(.2,14,10),mats.gold),shade=new THREE.Mesh(new THREE.CylinderGeometry(.34,.22,.25,12,1,true),mats.dark);pole.position.y=.85;bulb.position.y=1.72;shade.position.y=1.88;lamp.add(pole,bulb,shade);lamp.position.set(x+1.8,y,z);scene.add(lamp);const light=new THREE.PointLight(0xffb51f,8,13,1.55);light.position.set(x+1.8,y+1.7,z);light.castShadow=index%3===0;light.shadow.mapSize.set(512,512);scene.add(light)});
...
function makeCat(){const g=new THREE.Group();const body=new THREE.Mesh(new RoundedBoxGeometry(1.2,1.3,1.08,5,.18),mats.pink);body.castShadow=true;g.add(body);const muzzle=new THREE.Mesh(new RoundedBoxGeometry(.72,.47,.06,4,.1),mats.palePink);muzzle.position.set(0,.02,.55);g.add(muzzle);for(const x of[-.24,.24]){const eye=new THREE.Mesh(new THREE.SphereGeometry(.055,10,8),mats.dark);eye.position.set(x,.15,.59);g.add(eye);const ear=new THREE.Mesh(new THREE.ConeGeometry(.24,.48,4),mats.deepPink);ear.position.set(x*1.55,.87,0);ear.rotation.y=Math.PI/4;g.add(ear)}for(const x of[-.73,.73]){const arm=new THREE.Mesh(new RoundedBoxGeometry(.25,.62,.3,4,.1),mats.pink);arm.position.set(x,-.08,0);arm.rotation.z=x<0?-.12:.12;arm.castShadow=true;g.add(arm)}const band=new THREE.Mesh(new RoundedBoxGeometry(1.29,.17,1.17,3,.06),mats.deepPink);band.position.y=.38;g.add(band);return g}
